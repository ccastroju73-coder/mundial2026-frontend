...
export default function LineupComponent({ homePlayers = [], awayPlayers = [], homeBench = [], awayBench = [], match, teams = {} }) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-[#05070a] text-white">
      ...
      <div className="text-center pb-6 mb-8 bg-[#0f1115] p-8 rounded-3xl border border-white/10 shadow-2xl">
        <div className="flex justify-center items-center gap-12 mb-6">
          <div className="flex flex-col items-center gap-4">
            <img src={homeTeamData.flag} className="w-20 h-14 object-cover rounded-md shadow-lg" />
            <span className="text-2xl font-bold">{homeTeamData.name_en}</span>
          </div>
          <div className="text-5xl font-black font-mono text-white/90">
            {match.home_score ?? 0} - {match.away_score ?? 0}
          </div>
          <div className="flex flex-col items-center gap-4">
            <img src={awayTeamData.flag} className="w-20 h-14 object-cover rounded-md shadow-lg" />
            <span className="text-2xl font-bold">{awayTeamData.name_en}</span>
          </div>
        </div>
        
        {/* Color verde aplicado aquí */}
        <p className="text-sm font-bold uppercase tracking-widest text-[#22c55e]">
          {isFinished ? 'FINALIZADO' : isLive ? 'EN VIVO' : 'PRÓXIMO PARTIDO'}
        </p>
      </div>
      
      {/* Alineaciones con títulos en blanco */}
      ...
      {/* Suplentes con títulos en blanco */}
      ...
      {/* Pie de página */}
      <div className="pt-6 border-t border-white/10 flex justify-between text-xs text-gray-500 uppercase">
