...
export default function LineupComponent({ homePlayers = [], awayPlayers = [], homeBench = [], awayBench = [], match, teams = [] }) {
  // --- LOG DE DIAGNÓSTICO ---
  console.log("LineupComponent - Match recibido:", match);
  console.log("LineupComponent - Teams recibidos:", teams);
  ...
  // --- LOG DE ESTADO ---
  console.log("Status detectado:", match.status);

  // Mejoramos la lógica: Si status existe, lo usamos. 
  // Agregamos una verificación extra por si viene vacío o nulo.
  const isFinished = match.status === 'finished' || match.status === 'Finalizado';
  const isLive = match.status === 'live' || match.status === 'En curso';
