...
export default function LineupComponent({ homePlayers = [], awayPlayers = [], homeBench = [], awayBench = [], match, teams = {} }) {
  ...
  // Aquí usamos el objeto 'coaches' que importaste arriba
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-[#05070a] text-white">
      ...
      {/* Cabecera */}
      <div className="flex justify-between items-center bg-[#0f1115] p-8 rounded-3xl border border-white/10 shadow-2xl mb-8">
        <div className="flex flex-col items-center gap-4">
          <img src={homeTeamData.flag} className="w-20 h-14 object-cover rounded-md shadow-lg" />
          <span className="text-2xl font-bold tracking-tight">{homeTeamData.name_en}</span>
        </div>
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-2xl font-bold tracking-tight">{awayTeamData.name_en}</span>
        </div>
      </div>
      
      {/* Alineaciones Titulares */}
      <div className="bg-[#0b0e14] rounded-2xl p-6 border border-white/5 shadow-2xl mb-6">
        <h2 className="text-gray-400 text-center text-sm font-semibold mb-6">FORMACIÓN INICIAL</h2>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-[#0b0e14] rounded-2xl p-4 border border-white/5">
          <h3 className="text-xs text-gray-500 text-center mb-3">SUPLENTES LOCAL</h3>
          {homeBench.map((p) => <div key={p.id} className="text-sm text-gray-300 py-1">{p.name}</div>)}
        </div>
        <div className="bg-[#0b0e14] rounded-2xl p-4 border border-white/5">
          <h3 className="text-xs text-gray-500 text-center mb-3">SUPLENTES VISITANTE</h3>
          {awayBench.map((p) => <div key={p.id} className="text-sm text-gray-300 py-1 text-right">{p.name}</div>)}
        </div>
      </div>

      {/* Footer de DTs */}
      <div className="mt-10 pt-6 border-t border-gray-700 flex justify-between text-sm text-gray-500">
